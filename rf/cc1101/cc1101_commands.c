...
static const ShellCommandItem init_command_items[] = {
  ...
  {nullptr, param_handler, nullptr}, // frequency
  {nullptr, nullptr, init_handler}
};
static const ShellCommand init_command = {
  init_command_items,
  "cc1101_init",
  "cc1101_init host_address packet_length frequency",
  nullptr,
  nullptr
};
...
static const ShellCommandItem power_down_command_items[] = {
  {nullptr, nullptr, power_down_handler}
};
...
static int send_handler(printf_func pfunc, gets_func gfunc, int argc, char **argv, void *data)
{
  ...
  int rc = read_hex_string(argv[1], data_to_send, packet_length);
  if (rc <= 0)
  {
    pfunc("Invalid data\r\n");
    return 3;
  ...
  rc = cc1101Send(address, data_to_send, packet_length);
  if (rc)
    return rc;
  return cc1101PowerDown();
}

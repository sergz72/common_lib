...
static int status2_handler(printf_func pfunc, gets_func gfunc, int argc, char **argv, void *data);
static const ShellCommandItem status2_command_items[] = {
  {nullptr, nullptr, status2_handler}
};
static const ShellCommand status2_command = {
  status2_command_items,
  "pcf8563_status2",
  "pcf8563_status2",
  nullptr,
  nullptr
...
static int stop_handler(printf_func pfunc, gets_func gfunc, int argc, char **argv, void *data);
static const ShellCommandItem stop_command_items[] = {
  {nullptr, nullptr, stop_handler}
};
static const ShellCommand stop_command = {
  stop_command_items,
  "pcf8563_stop",
  "pcf8563_stop",
  nullptr,
...
static int start_handler(printf_func pfunc, gets_func gfunc, int argc, char **argv, void *data);
static const ShellCommandItem start_command_items[] = {
  {nullptr, nullptr, start_handler}
};
static const ShellCommand start_command = {
  start_command_items,
  "pcf8563_start",
  "pcf8563_start",
  nullptr,
...
{
  unsigned char status;
  int rc = pcf8563_get_status2(&status);
  if (rc != 0)
    return rc;
  pfunc("Status2: %02X\r\n", status);
  return 0;
...
void register_pcf8563_commands(void)
{
  ...
  shell_register_command(&status2_command);
